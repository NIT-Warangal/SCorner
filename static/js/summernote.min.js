!function (a) {
  ...
  'codemirror'], a)  : a(window.jQuery, window.CodeMirror)
}(function (a, b) {
  ...
  e = function () {
    ...
    i = function (a) {
      for (var b = [
      ], c = 0, d = a.length; d > c; c++) a[c] && b.push(a[c]);
      return b
    };
    return {
      head: a,
  ...
  f = function () {
    ...
    n = function (a, b) {
      ...
      f = function (g) {
        if (g) {
          if (g === a && (d = !0), d && !e && c.push(g), g === b) return e = !0,
          void 0;
    ...
    o = function (a, b) {
      b = b || d.fail;
      for (var c = [
      ]; a && (c.push(a), !b(a)); ) a = a.previousSibling;
      return c
    },
    p = function (a, b) {
      b = b || d.fail;
      for (var c = [
      ]; a && (c.push(a), !b(a)); ) a = a.nextSibling;
      return c
    },
    q = function (a, b) {
      var c = b.nextSibling,
  ...
  i = function () {
    ...
    this.current = function (b, c) {
      ...
      return e.image = f.isImg(c) && c,
      e.anchor = b.isOnAnchor() && f.ancestor(b.sc, f.isAnchor),
      e.aAncestor = f.listAncestor(b.sc, f.isEditable),
      e
    }
  },
  j = function () {
    ...
    d = function (a, b) {
      ...
      g.createFromBookmark(a[0], b.bookmark).select()
    };
  ...
  k = function () {
    ...
    ], j = 0, k = e.length; k > j; j++) this[e[j]] = function (a) {
      return function (b, c) {
        d(b),
    ...
    this.insertImage = function (b, c) {
      h.loadImage(c).done(function (e) {
        ...
        f.css('width', Math.min(b.width(), e.width)),
        g.create().insertNode(f[0])
      }).fail(function () {
    ...
    this.insertTable = function (b, e) {
      d(b);
      for (var f, h = e.split('x'), i = h[0], j = h[1], k = [
      ], l = c.bMSIE ? '&nbsp;' : '<br/>', m = 0; i > m; m++) k.push('<td>' + l + '</td>');
      f = k.join('');
      for (var n, o = [
      ], p = 0; j > p; p++) o.push('<tr>' + f + '</tr>');
      n = o.join('');
  ...
  l = function () {
    this.update = function (b, c) {
      ...
      e.find('.note-current-fontsize').html(c['font-size']),
      d(e, parseFloat(c['font-size']));
  ...
  o = function () {
    this.showImageDialog = function (b, c, d, e) {
      ...
      f.on('shown.bs.modal', function () {
        ...
        g.off('change'),
        f.off('shown.bs.modal hidden.bs.modal'),
        h.off('keyup'),
        i.off('click')
      }).modal('show')
    },
    this.showVideoDialog = function (a, b, c) {
      ...
      d.on('show.bs.modal', function () {
        ...
        e.off('keyup'),
        f.off('click'),
        d.off('show.bs.modal hidden.bs.modal')
      }).modal('show')
    },
    this.showLinkDialog = function (a, b, c) {
      ...
      d.on('shown.bs.modal', function () {
        e.html(b.text),
        f.val(b.url).keyup(function () {
          f.val() ? g.removeClass('disabled').attr('disabled', !1)  : g.addClass('disabled').attr('disabled', !0),
          b.text || e.html(f.val())
        }).trigger('focus'),
        g.click(function (a) {
          d.modal('hide'),
        ...
        f.off('keyup'),
        g.off('click'),
        d.off('shown.bs.modal hidden.bs.modal')
      }).modal('show')
    },
  ...
  p = function () {
    ...
    r = function (a) {
      ...
      if (f === p.TAB && h.editable().data('tabsize')) d.tab(h.editable());
       ...
       else if (b && f === p.K) h.editable(),
      d.setLinkDialog(h.editable(), function (a, b) {
        ...
        if (!b || f !== p.ENTER) return (f === p.BACKSPACE || f === p.ENTER || f === p.SPACE) && d.recordUndo(q(a.target).editable()),
        void 0;
    ...
    z = function (g) {
      ...
      if (h.length > 0) {
        ...
        if (d[k] && (r.trigger('focus'), d[k](r, l, i)), - 1 !== a.inArray(k, [
          'backColor',
          'foreColor'
        ])) e.updateRecentColor(h[0], k, l);
         else if ('showLinkDialog' === k) r.focus(),
        d.setLinkDialog(r, function (a, b) {
          j.showLinkDialog(p, a, b)
        });
         else if ('showImageDialog' === k) r.focus(),
        j.showImageDialog(p, t, function (a) {
          s(r, a)
        }, function (a) {
          d.restoreRange(r),
          d.insertImage(r, a)
        });
         else if ('showVideoDialog' === k) r.focus(),
        d.setVideoDialog(r, function (a, b) {
          ...
          if (z) r.data('orgHeight', r.css('height')),
          a(window).resize(y).trigger('resize');
           else {
            var A = !!r.data('optionHeight');
            r.css('height', A ? r.data('orgHeight')  : 'auto'),
            a(window).off('resize')
          }
          ...
          if (B) {
            if (u.val(r.html()), u.height(r.height()), e.disable(o), u.focus(), c.bCodeMirror) {
              ...
              C.setSize(null, r.outerHeight()),
              C.autoFormatRange && C.autoFormatRange({
                line: 0,
                ch: 0
              }, {
                line: C.lineCount(),
          ...
          } else c.bCodeMirror && u.data('cmEditor').toTextArea(),
          r.html(u.val() || f.emptyPara),
          r.height(r.data('optionHeight') ? u.height()  : 'auto'),
          e.enable(o),
          r.focus();
    ...
    this.attach = function (a, b) {
      a.editable.on('keydown', r),
      a.editable.on('mousedown', u),
      a.editable.on('keyup mouseup', v),
      a.editable.on('scroll', w),
      E(a),
      a.handle.on('mousedown', x),
      a.toolbar.on('click', z),
      a.popover.on('click', z),
      a.toolbar.on('mousedown', y),
      a.popover.on('mousedown', y),
      a.statusbar.on('mousedown', B);
      var c = a.toolbar,
  ...
  r = function () {
    ...
    this.createLayout = function (c, i) {
      ...
      if (!c.next().hasClass('note-editor')) {
        ...
        n.data('options', i),
        k > 0 && a('<div class="note-statusbar">' + h + '</div>').prependTo(n);
        var o = a('<div class="note-editable" contentEditable="true"></div>').prependTo(n);
        k && (o.height(k), o.data('optionHeight', k)),
        l && o.data('tabsize', l),
        o.html(f.html(c) || f.emptyPara),
        o.data('NoteHistory', new j),
        a('<textarea class="note-codable"></textarea>').prependTo(n),
        setTimeout(function () {
        ...
        a('<div class="note-dropzone"><div class="note-dropzone-message"></div></div>').prependTo(n),
        n.insertAfter(c),
        c.hide()
      }
    };
    ...
    code: function (b) {
      if (void 0 === b) {
        ...
        if (e && e.editable) {
          ...
          return c.bCodeMirror && e.codable.data('cmEditor').save(),
